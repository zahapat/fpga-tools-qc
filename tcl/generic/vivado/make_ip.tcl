...
# set_property -name "ip_output_repo" -value "${orig_proj_dir}/ip/$IPname_0" -objects [current_project]
reset_project
...
# set fileset_sources_name ${IPname_0}_sources_1
# set fileset_sim_name ${IPname_0}_sim_1
# set fileset_constrs_name ${IPname_0}_constrs_1
...
if {$file_type eq "vhd"} {
    set grp_synth [ipx::add_file_group -type vhdl:synthesis {} [ipx::current_core]]
    set grp_sim_mixed [ipx::add_file_group -type simulation {} [ipx::current_core]]
    set_property target_language VHDL [current_project]
    puts "TCL: Target language = VHDL"
} elseif {$file_type eq "v"} {
    set grp_synth [ipx::add_file_group -type verilog:synthesis {} [ipx::current_core]]
    set grp_sim_mixed [ipx::add_file_group -type simulation {} [ipx::current_core]]
...
report_compile_order -fileset $fileset_sources_name -used_in synthesis -missing_instance -of [get_ips $newTop] -file "$IPdir/0_ip_report_modules_missing.rpt"
set missingFiles [report_compile_order -fileset $fileset_sources_name -used_in synthesis -missing_instance -of [get_ips $newTop]]
...
# set grp_synth_sv [ipx::add_file_group -type verilog:synthesis {} [ipx::current_core]]
# set grp_sim_mixed [ipx::add_file_group -type simulation {} [ipx::current_core]]
foreach f $listOfFiles_sources_1 {
...
# ----------------------------
# Prerequisites: make_ip_pack must run before this script and output files muts be generated
# Vivado project:
    # Create XCI by instantiating component just added to "IP Integrator Library"
set xml_dir "[file normalize $IPdir]"
...
# update_ip_catalog
set core [ipx::open_core -set_current false $xml_dir]
ipx::unload_core $core

    #   </File>
    #   <File Path="$PPRDIR/../ip/ip_pack_0/ip_pack_0/ip_pack_0.xci">
    #     <FileInfo>
...
create_ip -verbose -module_name ${IPname_0} -vendor patrik_zahalka.com -library ip -name ${IPname_0} -version 1.0 -dir "${orig_proj_dir}/ip/$IPname_0"
set ip_obj [get_ips "${IPname_0}"]
# generate_target -help
# generate_target {all} $ip_obj

#     # Synthesize core and generate all output files
save_project_as -force ${IPname_0} $IPdir
generate_target {all} $ip_obj
# set run_name "${IPname_0}_synth_1"
# set run_jobs 8
# puts "TCL: Synthesize IP: name = ${run_name} jobs = ${run_jobs}"
# create_ip_run $ip_obj
# launch_run -jobs $run_jobs $run_name
# wait_on_run $run_name
# generate_target {instantiation_template synthesis} [get_ips [current_run]]
# puts "TCL: .XCI file has been exported here: $IPdir"
